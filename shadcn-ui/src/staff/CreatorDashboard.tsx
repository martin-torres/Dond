    ...
    const loadStats = async () => {
      try {
        ...
        if (restaurantsResult.status === 'fulfilled') {
          ...
          totalOrders = restaurantsResult.value.data.reduce((sum: number, r: unknown) =>
            sum + (r.total_orders || 0), 0);
          totalRevenue = restaurantsResult.value.data.reduce((sum: number, r: unknown) =>
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                <h2 className="text-xl font-semibold mb-2">
                  {adminDashboardTabs.find((tab: unknown) => tab.id === activeTab)?.label}
                </h2>
