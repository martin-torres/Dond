...
export const BarView = () => {
  ...
  // Filter to show only drink items that need bar attention
  const barItems = useMemo(() => {
    ...
    orders.forEach((order) => {
      ...
      drinkItems.forEach((item) => {
        // Only show items that need bar attention
  ...
  const handleStartItem = async (itemId: string) => {
    try {
      ...
      console.error('Failed to start drink item:', error);
    }
  };
  ...
  const handleItemReady = async (itemId: string) => {
    try {
      ...
      console.error('Failed to mark drink item as ready:', error);
    }
  };
  ...
  return (
    <StaffLayout
      title="Bar stations"
      subtitle="Drink orders organized by table location. Each item has independent status."
      hideNav
    >
      <div className="h-full overflow-y-auto">
        {barItems.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-sm text-gray-500">No active bar items. New drink orders will appear here organized by table.</p>
          </div>
        ) : (
                ...
                <div key={tableKey} className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex-shrink-0" style={{ width: '320px' }}>
                  ...
                  <div className="bg-blue-50 px-3 py-2 border-b border-blue-100">
                    ...
                    <p className="text-xs text-gray-600">
                      {itemsForTable.length} drink item{itemsForTable.length !== 1 ? 's' : ''}
                    </p>
                  </div>
                          ...
                          <div className="flex gap-2">
                            {actions.map((action) => (
                              <button
                                ...
                                className="flex-1 bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold py-1 px-2 rounded transition-colors"
                              >
                                {action.label}
                              </button>
                            ))}
