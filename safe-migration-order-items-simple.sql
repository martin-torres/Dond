-- Simple safe migration script for order_items table
-- This version uses individual statements that are more compatible with Supabase

-- 1. Add status column if it doesn't exist
ALTER TABLE IF EXISTS order_items ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'NEW' NOT NULL;

-- 2. Add status constraint if it doesn't exist
ALTER TABLE IF EXISTS order_items ADD CONSTRAINT IF NOT EXISTS order_items_status_check 
CHECK (status IN ('NEW', 'IN_PROGRESS', 'READY', 'PICKING_UP', 'DELIVERED'));

-- 3. Add assigned_station column if it doesn't exist
ALTER TABLE IF EXISTS order_items ADD COLUMN IF NOT EXISTS assigned_station TEXT;

-- 4. Add assigned_station constraint if it doesn't exist
ALTER TABLE IF EXISTS order_items ADD CONSTRAINT IF NOT EXISTS order_items_station_check 
CHECK (assigned_station IN ('kitchen', 'bar', 'server'));

-- 5. Add status_updated_at column if it doesn't exist
ALTER TABLE IF EXISTS order_items ADD COLUMN IF NOT EXISTS status_updated_at TIMESTAMPTZ DEFAULT NOW();
...
-- Success message
SELECT 'Safe migration complete! Item-level status system is ready.' as message;
